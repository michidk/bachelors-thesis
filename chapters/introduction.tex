...
In order to immerse the user in a so-called \gls{VE}, a \gls{HMD} as the display device has to be connected to a \gls{PC}. The \gls{PC}, which processes data from input devices like motion controllers or motion data from the \gls{HMD}, is required for most consumer \gls{VR} systems. Often an external tracking system is presumed, too. An application, rendering three-dimensional content to the \gls{HMD}, similar to a computer game rendering to a regular display, is required to present the \gls{VE} to the user.
