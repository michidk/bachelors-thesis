...
The implementation of this experiment listens for new clients. As soon as one connects, a new \ac{UBII} interaction is published and the resulting \ac{UBII} topic is subscribed. Since the smart device (see Section~\ref{section:smart-device}) publishes the orientation data in a different format than ThreeJS needs for rendering, a reusable \ac{UBII} interaction was created. The \ac{UBII} interaction converts the angles from radian to degrees, changes the coordinate system and publishes them to the \lstinline[breaklines=false]{[client id]/SAVRLaserPointer/orientation}-topic. The code for the \ac{UBII} interaction is shown in Figure~\ref{fig:ubii-interaction-angles}.
...
In addition to the orientation \ac{UBII} topic, this implementation subscribes to the touch events \ac{UBII} topic. The button down event is needed to trigger the actual selection.
...
Text input is not an easy task to perform in \ac{VR}. But it is often required for labeling, annotation, entering filenames for saving operations and setting parameters in visualizations~\cite[2154]{Rhoton.2002}. This is why a lot of applications try to avoid it. 

Tilt Brush\footnote{Tilt Brush by Google is a tool for three-dimensional painting in VR. Website: \href{https://www.tiltbrush.com/}{www.tiltbrush.com}} solves this by identifying their scenes with a screenshot of the scene rather than a file name. To save a scene, the users gets a virtual camera attached to his hand motion controller, which he then uses to make a thumbnail of the current scene~\cite{GoogleLLC.2019}. 
...
The \lstinline{TextDisplay} component just renders a given text inside a given area. If the text is changed it automatically updates and redraws the texture.